import {
	initialCreateValidationString,
	constructorComments,
	createMethodComments,
	toOriginalMethodComment,
} from "./values";
...
export class Table {
	...
	getClassDefinitionString(config: Configuration): string {
		...
		output.push("");
		output.push(this.getPrimaryKeys());
	...
	private getPrimaryKeys(indent = 1): string {
		const PRI_KEYS: string[] = [];
		for (const column of this.columns) {
			if (column.key === "PRI") PRI_KEYS.push(column.name);
		}
		const output = `${" ".repeat(
			indent * 4
		)}static get PK() {\n${" ".repeat(
			(indent + 1) * 4
		)}return '${PRI_KEYS.toString()}';\n${" ".repeat(indent * 4)}}`;
		return PRI_KEYS.length ? output : "";
	}
	...
	private getConstructor(config: Configuration, indent = 1): string {
		...
		if (config.Mode === "basic") {
			...
			output.push(
				" ".repeat(indent * 4) + `private constructor(obj: any) {`
			);
		}
		...
		for (const column of this.columns) {
			output.push(
				" ".repeat((indent + 1) * 4) +
					`this.${formatName(config, column.name)} = obj.${
						column.name
					};`
			);
		}
	...
	private getCreateMethod(indent = 1): string {
		...
		output.push(
			" ".repeat(indent * 4) +
				`public static Create(obj: any, field: string = 'root'): ${upperFirst(
					this.name
				)} {`
		);
		...
		for (const column of this.columns) {
			output.push(
				" ".repeat((indent + 1) * 4) +
					`check${upperFirst(column.type)}(obj.${column.name}, ${
						column.nullable
					}, field + '.${column.name}');`
			);
		}
		...
		output.push(
			" ".repeat((indent + 1) * 4) +
				`return new ${upperFirst(this.name)}(obj);`
		);
		output.push(" ".repeat(indent * 4) + "}");
	...
	private getToOriginalMethod(config: Configuration, indent = 1): string {
		...
		output.push(
			" ".repeat(indent * 4) +
				`public originalObject(json: boolean = false): any {`
		);
		...
		for (const col of this.columns) {
			output.push(
				" ".repeat((indent + 1) * 4) +
					`output.${col.name} = this.${formatName(config, col.name)};`
			);
		}
		...
		output.push(
			" ".repeat((indent + 2) * 4) + "return JSON.stringify(output);"
		);
